...
export let loginLoader: LoaderFunction = async ({ request }) => {
  ...
  let magicToken = searchParams.get("key");
  ...
  if (response.ok) {
    let validationResponse = await response.json();
    ...
    session.set("token", validationResponse.token);

    /**
     * TODO: Default the landingPage value
     */
    return redirect(validationResponse.landingPage, {
      headers: {
        "Set-Cookie": await authSession.commitSession(session, {
          maxAge: sessionMaxAge,
        }),
