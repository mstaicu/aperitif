...
test("isReserved returns the correct status when an order gets created for that ticket", async () => {
  const ticket = Ticket.build({
    id: new mongoose.Types.ObjectId().toHexString(),
    title: "concert",
    price: 5,
  ...
  expect(isTicketReserved).toBe(true);
});

test("isReserved returns the correct status when an order gets cancelled for that ticket", async () => {
  const ticket = Ticket.build({
    id: new mongoose.Types.ObjectId().toHexString(),
    title: "concert",
    price: 5,
  ...
  expect(isTicketReserved).toBe(false);
});
