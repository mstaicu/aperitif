 ...
 * @param {{admissions: AdmissionsRuntime, jwks: Jwks, spaces: SpacesRuntime}} opts
 */
export default async function (fastify, { admissions, jwks, spaces }) {
  await fastify.register(list, { jwks, spaces });
  await fastify.register(get, { jwks, spaces });
  await fastify.register(destroy, { jwks, spaces });
  await fastify.register(createAdmission, { admissions, jwks });
