...
```text
Kubernetes -> NATS servers -> streams -> consumers -> Outbox Relays/projections
```
...
- Missing streams usually means the owning Outbox Relay has not started or
...
```text
domain transaction -> outbox_events -> Outbox Relay -> stream
                   -> consumer -> projection -> projection table
```

```sh
kubectl logs -n accounts deployment/outbox-relay --since=10m
kubectl logs -n plans deployment/outbox-relay --since=10m
kubectl logs -n plans deployment/accounts-projection --since=10m
```

An unpublished outbox entry points to the Outbox Relay or NATS connection. A
